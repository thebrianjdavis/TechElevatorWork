...
namespace Exercises
{
    public partial class Exercises
    {
        ...
        */
        ...
        {
            List<string> replaceFizzBuzzList = new List<string>();
            
            for (int i = 0; i < integerArray.Length; i++)
            {
                if (integerArray[i] % 3 == 0 && integerArray[i] % 5 == 0)
                {
                    replaceFizzBuzzList.Add("FizzBuzz");
                }
                else if (integerArray[i] % 3 == 0)
                {
                    replaceFizzBuzzList.Add("Fizz");
                }
                else if (integerArray[i] % 5 == 0)
                {
                    replaceFizzBuzzList.Add("Buzz");
                }
                else
                {
                    string fromInt = "";
                    fromInt = integerArray[i].ToString();
                    replaceFizzBuzzList.Add(fromInt);
                }
            }
            
            return replaceFizzBuzzList;
        }
    }
}
