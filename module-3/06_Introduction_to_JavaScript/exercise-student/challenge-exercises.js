...
function titleCase(str, minorWords = "") {
    ...
    let sentence = lowercaseStr.split(' ');
    let lowercaseWords = minorWords.toLowerCase();
    let checkWords = lowercaseWords.split(' ');

    for (let index = 0; index < sentence.length; index++) {
        
        if (checkWords.includes(sentence[index]) && index != 0) {
            sentence[index] = sentence[index].charAt(0).toLowerCase() + sentence[index].slice(1);
            titleCaseResult += " " + sentence[index];
        }
        else if (index == 0) {
            sentence[index] = sentence[index].charAt(0).toUpperCase() + sentence[index].slice(1);
            titleCaseResult += sentence[index];
        }
        else {
            sentence[index] = sentence[index].charAt(0).toUpperCase() + sentence[index].slice(1);
